...
export default function HeritageCategoryContent({ slug }: { slug: string }) {
  const { state } = useCms();
  ...
  const styles = colorStyles[cat.color] || colorStyles.gold;

  // Adjacent categories for navigation
  const idx = categories.findIndex((c) => c.slug === slug);
  const prev = categories[idx - 1] ?? null;
    ...
    <div className="bg-[var(--color-bg-secondary)] overflow-x-hidden">
      ...
      <div className="bg-[var(--color-heritage-black)] text-white py-20 px-6 relative overflow-hidden">
        <div className="absolute inset-0">
          {cat.imageUrl && <Image src={cat.imageUrl} alt={cat.name} fill priority className="object-cover opacity-20" />}
          ...
          <div className="space-y-8">
            {(cat.items || []).map((item, i) => (
              ...
              >
                ...
                >
                  {/* Number + image column */}
                  <div className="lg:col-span-2 relative min-h-[220px] bg-neutral-550">
                    {item.imageUrl ? (
                      <Image
                        src={item.imageUrl}
                        alt={item.name}
