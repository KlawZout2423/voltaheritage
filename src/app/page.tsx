    ...
    <section id="services" className="py-24 bg-[var(--color-heritage-black)]">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {cmsServices.slice(0, 6).map((svc, i) => (
            <AnimateOnScroll 
              key={svc.id} 
              direction="up" 
              delay={i * 0.1}
  ...
  return (
    <div className="bg-[var(--color-bg-secondary)] overflow-x-hidden">
      {sectionsOrder.map((sectionId) => {
        ...
        switch (sectionId) {
          case "hero":
            return <Fragment key={sectionId}>{renderHero()}</Fragment>;
          case "events":
            return <Fragment key={sectionId}>{renderEvents()}</Fragment>;
          case "heritage":
            return <Fragment key={sectionId}>{renderHeritage()}</Fragment>;
          case "about":
            return <Fragment key={sectionId}>{renderAbout()}</Fragment>;
          case "services":
            return <Fragment key={sectionId}>{renderServices()}</Fragment>;
          case "news":
            return <Fragment key={sectionId}>{renderNews()}</Fragment>;
          case "connect":
          case "cta-banner":
            return <Fragment key={sectionId}>{renderConnect()}</Fragment>;
          default:
            return null;
        }
      })}
